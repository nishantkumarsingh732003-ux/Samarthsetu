...
export function ApplyClient({
  ...
  const submit = useCallback(async () => {
    ...
    if (result.ok) {
      // `/applications/[ref]`, not the old `/track/[ref]`. Tracking by reference number
      // with no login was the anonymous journey's ending; that journey is gone, and the
      // signed-in application detail screen shows the same status and the same document
      // checklist against an account that already knows who this is.
      router.push(`/${locale}/applications/${result.data.reference_no}`);
      return;
    }
    setSubmitting(false);
  ...
  return (
    ...
    >
      ...
      {checklist && checklist.length > 0 ? (
        <section className="panel p-4" aria-labelledby="bring-heading">
