...
| OI-62 | `docker-compose` mounted `./apps/web` into the web container and masked `node_modules` with an anonymous volume, but not `.next`. Running `pnpm --filter @setu/web check` on the host therefore wrote a **production** build into the directory the container's `next dev` was serving from, and the dev server handed the browser its own dev manifest pointing at production chunks: `TypeError: Cannot read properties of undefined (reading 'call')` out of webpack.js, with a clean server log. `.next` is now masked the same way, for the same reason a Linux container cannot share a host install. | 2026-09-04 |
| OI-63 | The service worker cached every same-origin GET **cache-first**, justified by "they are content-hashed, so a stale hit is correct". True of `/_next/static` in a production build; false of icons and the manifest, and entirely false in development, where `next dev` serves `/_next/static/chunks/webpack.js` unhashed. A browser that visited the dev server once pinned that webpack runtime against every later build — the same `reading 'call'` crash, surviving a hard reload, because the worker answers before the network. Cache-first is now limited to `/_next/static`; everything else is network-first with a cache fallback; the worker is not registered in development and is actively unregistered there. | 2026-09-04 |
| OI-64 | Fixing OI-63 in a `useEffect` could not heal an already-broken browser: a stale chunk crashes the page before React mounts, so the cleanup never runs and a hard reload does not reach it. The thing that undoes a broken worker cannot be delivered by the worker. A synchronous inline script in the layout — served with the HTML, which navigations fetch network-first — now unregisters, purges and reloads once, guarded on a registration existing so it cannot loop. Verified by poisoning a cache in a real browser and counting document loads: one, either way. | 2026-09-04 |
